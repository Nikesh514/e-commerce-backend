...
export const order_confirmation_html = (items:any[],totalAmount:number,user:any,order:any,req:any) =>{
    const html = `
  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; padding: 20px; border-radius: 8px; background: #f9f9f9;">
    ...
    <p style="font-size: 16px; color: #333;">Hi ${user.full_name ?? 'Customer'},</p>
    ...
    <p style="margin: 8px 0;">${order.shipping_address ?? 'Not provided'}</p>
